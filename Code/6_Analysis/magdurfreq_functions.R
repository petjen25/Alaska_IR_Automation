...
for(i in Unique_AUIDs){
  ...
  for(j in 1:nrow(my_data_magfreqdur)) {
    ...
    if(filter_by$Directionality == 'Maximum' & filter_by$Frequency == 'Not to exceed' &
      ...
      bad_tot <- results %>% select(day_row, bad_year) %>% unique()
      bad_sum <- sum(bad_tot$bad_year)
      ...
      bad_tot <- results %>% select(num_samples, bad_year) %>% unique()
      bad_sum <- sum(bad_tot$bad_year)
      ...
      results <- filt %>%
        group_by(ActivityStartDate) %>%
        mutate(roll_1day_mean = map_dbl(ActivityStartDateTime, 
                                        ~mean(TADA.ResultMeasureValue[between(ActivityStartDateTime, .x - hours(24), .x)])), 
               bad_samp = ifelse(daily_mean >= filter_by$Magnitude_Numeric, 1, 0)) 
      ...
      results <- filt %>%
        ...
        mutate(roll_4day_mean = map_dbl(ActivityStartDateTime, 
                                        ~mean(TADA.ResultMeasureValue[between(ActivityStartDateTime, .x - hours(96), .x)])), 
      ...
      results <- joined %>%
        filter(year >= max_year - 3) %>%
        group_by(ActivityStartDate) %>%
        mutate(avg_1hr = map_dbl(ActivityStartDateTime, 
                                 ~mean(TADA.ResultMeasureValue[between(ActivityStartDateTime, .x - hours(1), .x)])),
               bad_samp = ifelse(avg_1hr >= magnitude, 1, 0))
      ...
      results <- filt %>%
        ...
        mutate(avg_1hr = map_dbl(ActivityStartDateTime, 
                                 ~mean(TADA.ResultMeasureValue[between(ActivityStartDateTime, .x - hours(1), .x)])), 
               bad_samp = ifelse(avg_1hr >= filter_by$Magnitude_Numeric, 1, 0)) 
      ...
      if(filter_by$Constituent == 'Ammonia'){
        ...
        joined <- filt %>%
          ...
          mutate(magnitude = exp(0.819*(log(Hardness))+0.6848)*0.860) #Table from Toxics manual - Appendix G
